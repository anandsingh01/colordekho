...
{{--                </form>--}}
                ...
                <div class="body">
                    <form method="post" action="{{url('admin/save-cars-color')}}" class="category_form"
                    enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="category_name"><b> Car Model </b></label>
                            <select name="car_id" class="form-control ms select2">
                                @forelse($cars_manufacture as $cars_manufactures)
                                    <option value="{{$cars_manufactures->id}}">{{$cars_manufactures->name}}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
                ...
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            <tbody>
                            @if(!empty($cars_colors))
                                @foreach($cars_colors as $all_carsColor)
                                    <tr>
                                    <td>{{$all_carsColor->color}}</td>
                                    <td>{{$all_carsColor->get_cars[0]->name ?? ''}}</td>
                                    <td>
                                        @if($all_carsColor->status == 1)
                                            <span class="badge badge-info text-white" >Current status : Active</span><br>
                                            <span class="badge badge-danger">
                                                <a href="javascript:void(0)" data-id="{{$all_carsColor->id}}" data-status="0" class="status">
                                                    Change to : Inactive
                                                </a>
                                            </span>
                                        @else
                                            ...
                                            <span class="badge badge-info">
                                                <a href="javascript:void(0)" data-id="{{$all_carsColor->id}}" data-status="1" class="text-white status">
                                                    Change to : Active
                                                </a>
                                            </span>
                                        @endif
                                    ...
                                    <td>
                                        <a href="{{url('admin/edit-cars-color/'.$all_carsColor->id)}}" class="btn btn-success btn-sm btn-icon">
                                            <i class="zmdi zmdi-edit"></i></a>
                                        <button class="btn btn-sm btn-danger btn-icon" onclick="deleteConfirmation({{$all_carsColor->id}})">
        ...
        $(function() {
            $('.status').click(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{url("admin/update-cars-color-Status")}}',
        ...
        function deleteConfirmation(id) {
            swal({
                ...
                if (e.value === true) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: "{{url('admin/delete-cars-color')}}/" + id,
