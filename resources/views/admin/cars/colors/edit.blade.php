...
@section('body')
    ...
    <div class="row clearfix">
        <div class="col-lg-5 col-md-5 col-sm-12">
            <div class="card">
                <div class="body">
                    <form method="post" action="{{url('admin/update-cars-color')}}" class="category_form"
                    enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" value="{{$color->id}}">
                        <div class="form-group">
                            <label for="category_name"><b> Name</b></label>
                            <input name="name" value="{{$color->color}}" type="text" id="category_name" class="form-control" placeholder="Enter category">
                        </div>

                        <div class="form-group">
                            <label for="category_name"><b> Car Model </b></label>
                            <select name="car_id" class="form-control ms select2">
                                @forelse($all_cars as $all_carsColor)
                                    <option value="{{$all_carsColor->id}}" {{$color->car_id == $all_carsColor->id ? 'selected' : ''}}>{{$all_carsColor->name}}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
