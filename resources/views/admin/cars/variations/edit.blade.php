...
@section('body')
    ...
    <div class="row clearfix">
        <div class="col-lg-5 col-md-5 col-sm-12">
            <div class="card">
                <div class="body">
                    <form method="post" action="{{url('admin/update-cars-variations')}}" class="category_form"
                          enctype="multipart/form-data">
                        @csrf

                        <input type="hidden" name="id" value="{{$variations->id}}">

                        <div class="form-group">
                            <label for="category_name"><b> Variations </b></label>
                            <select name="variation" class="form-control ms select2">
                                <option value="500ml" {{$variations->variation === '500ml' ? 'selected' : ''}}>500ml</option>
                                <option value="1ltr" {{$variations->variation === '1ltr' ? 'selected' : ''}}>1 Ltr</option>
                                <option value="100ml" {{$variations->variation === '100ml' ? 'selected' : ''}}>100ml</option>
                        ...
                        <div class="form-group">
                            ...
                            <select name="color_id" class="form-control ms select2">
                                @forelse($all_colors as $all_colors)
                                    <option value="{{$all_colors->id}}" {{$variations->color_id == $all_colors->id ? 'selected' : ''}}>{{$all_colors->color}}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input name="price" type="number" value="{{$variations->price}}" id="price" class="form-control" placeholder="">
                        </div>

                        <div class="form-group">
                            <label for="category_name"><b> MRP </b></label>
                            <input name="mrp_price" value="{{$variations->mrp}}"  type="number" id="mrp_price" class="form-control" placeholder="">
                        </div>
