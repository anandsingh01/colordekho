...
{{--                </form>--}}
                ...
                <div class="body">
                    <form method="post" action="{{url('admin/save-cars-variations')}}" class="category_form"
                    enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="color_id" class="form-control ms select2">
                                @forelse($all_colors as $all_colors)
                                    <option value="{{$all_colors->id}}">{{$all_colors->color}}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input name="mrp_price" type="number" id="mrp_price" class="form-control" placeholder="">
                        </div>
                ...
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            <tbody>
                            @if(!empty($all_cars_variations))
                                @foreach($all_cars_variations as $all_cars_variation)
                                    <tr>
                                        <td>{{$all_cars_variation->variation}}</td>
                                        <td>{{$all_cars_variation->get_colors[0]->color ?? ''}}</td>
                                        <td>{{$all_cars_variation->price}}</td>

                                        <td>
                                        @if($all_cars_variation->status == 1)
                                            <span class="badge badge-info text-white" >Current status : Active</span><br>
                                            <span class="badge badge-danger">
                                                <a href="javascript:void(0)" data-id="{{$all_cars_variation->id}}" data-status="0" class="status">
                                                    Change to : Inactive
                                                </a>
                                            </span>
                                        @else
                                            ...
                                            <span class="badge badge-info">
                                                <a href="javascript:void(0)" data-id="{{$all_cars_variation->id}}" data-status="1" class="text-white status">
                                                    Change to : Active
                                                </a>
                                            </span>
                                        @endif
                                    ...
                                    <td>
                                        <a href="{{url('admin/edit-cars-variations/'.$all_cars_variation->id)}}" class="btn btn-success btn-sm btn-icon">
                                            <i class="zmdi zmdi-edit"></i></a>
                                        <button class="btn btn-sm btn-danger btn-icon" onclick="deleteConfirmation({{$all_cars_variation->id}})">
        ...
        $(function() {
            $('.status').click(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{url("admin/update-cars-variations-Status")}}',
        ...
        function deleteConfirmation(id) {
            swal({
                ...
                if (e.value === true) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: "{{url('admin/delete-cars-variations')}}/" + id,
