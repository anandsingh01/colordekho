...
{{--                </form>--}}
                ...
                <div class="body">
                    <form method="post" action="{{url('admin/save-bikes')}}" class="category_form"
                ...
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            <tbody>
                            @if(!empty($bike_manufacturer))
                                @foreach($bike_manufacturer as $bike_manufacturers)
                                    <tr>
                                    <td>{{$bike_manufacturers->name}}</td>
                                    <td>
                                        @if($bike_manufacturers->status == 1)
                                            <span class="badge badge-info text-white" >Current status : Active</span><br>
                                            <span class="badge badge-danger">
                                                <a href="javascript:void(0)" data-id="{{$bike_manufacturers->id}}" data-status="0" class="status">
                                                    Change to : Inactive
                                                </a>
                                            </span>
                                        @else
                                            ...
                                            <span class="badge badge-info">
                                                <a href="javascript:void(0)" data-id="{{$bike_manufacturers->id}}" data-status="1" class="text-white status">
                                                    Change to : Active
                                                </a>
                                            </span>
                                        @endif
                                    ...
                                    <td>
                                        <a href="{{url('admin/edit-bikes/'.$bike_manufacturers->id)}}" class="btn btn-success btn-sm btn-icon">
                                            <i class="zmdi zmdi-edit"></i></a>
                                        <button class="btn btn-sm btn-danger btn-icon" onclick="deleteConfirmation({{$bike_manufacturers->id}})">
        ...
        $(function() {
            $('.status').click(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{url("admin/update-bikes-Status")}}',
